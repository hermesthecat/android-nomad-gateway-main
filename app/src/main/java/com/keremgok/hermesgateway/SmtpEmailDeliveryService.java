...
public class SmtpEmailDeliveryService {
    ...
    public String createHtmlEmailFromActivityData(String eventType, JSONObject activityData) {
        ...
        try {
            ...
            htmlBody.append(
                    ".content { background-color: #f5f5f5; padding: 15px; border-radius: 5px; margin: 10px 0; }");
    ...
    private void createSmsHtmlContent(StringBuilder htmlBody, JSONObject data) {
        try {
            ...
            if (data.has("sentStamp")) {
                htmlBody.append("<p><strong>Sent:</strong> ").append(formatTimestamp(data.getString("sentStamp")))
                        .append("</p>");
            }
        } catch (Exception e) {
    ...
    private void createCallHtmlContent(StringBuilder htmlBody, JSONObject data) {
        try {
            ...
            if (data.has("timestamp")) {
                htmlBody.append("<p><strong>Time:</strong> ").append(formatTimestamp(data.getString("timestamp")))
                        .append("</p>");
            }
        } catch (Exception e) {
    ...
    private void createPushHtmlContent(StringBuilder htmlBody, JSONObject data) {
        try {
            ...
            if (data.has("package")) {
                htmlBody.append("<p><strong>App:</strong> ").append(escapeHtml(data.getString("package")))
                        .append("</p>");
            }

            if (data.has("title")) {
                htmlBody.append("<p><strong>Title:</strong> ").append(escapeHtml(data.getString("title")))
                        .append("</p>");
            }
            ...
            if (data.has("sentStamp")) {
                htmlBody.append("<p><strong>Time:</strong> ").append(formatTimestamp(data.getString("sentStamp")))
                        .append("</p>");
            }
        } catch (Exception e) {
    ...
    private void createGenericHtmlContent(StringBuilder htmlBody, JSONObject data) {
        ...
        try {
            ...
            htmlBody.append(escapeHtml(data.toString()));
        }
        htmlBody.append("</pre>");
    }
